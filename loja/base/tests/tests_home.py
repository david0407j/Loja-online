...
def test_pagina_deve_conter_o_link_produtos_masculinos(resp):
    assert_contains(resp, reverse('produtos:produto', kwargs={'nome_categoria':'Masculino'}))
    
def test_email(resp):
   assert_contains(resp, 'reizinhomp@gmail.com')

def test_pagina_o_link_infantil(resp):
    assert_contains(resp, reverse('produtos:produto', kwargs={'nome_categoria':'Infantil'}))

def test_pagina_deve_conter_o_link_produtos(resp):
    assert_contains(resp, reverse('produtos:produto', kwargs={'nome_categoria':'Produtos'}))

def test_pagina_deve_conter_o_link_feminina(resp):
    assert_contains(resp, reverse('produtos:produto', kwargs={'nome_categoria':'Feminina'}))

def test_pagina_deve_conter_o_link_cosmetico(resp):
    assert_contains(resp, reverse('produtos:produto', kwargs={'nome_categoria':'Cosmético'}))